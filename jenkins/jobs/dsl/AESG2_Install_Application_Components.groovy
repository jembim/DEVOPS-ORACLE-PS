...
def createJob = freeStyleJob(containerFolder + '/Install_Application_Components')
...
db_host=${DATABASE_HOST_IP} app_host=${APPLICATION_HOST_IP} \
database_sid=${CONTAINER_DATABASE_NAME} database_pdb_name=${PLUGGABLE_DATABASE_NAME} \
db_port=${DB_PORT} multi_architecture=${MULTITENANT}" --tags "install_app" ${MAIN}.yml
        ''')
    }
